...
#         conv3x3(in_planes, out_planes),
        SNConv2d(in_planes, out_planes, kernel_size=3, stride=1, padding=1, bias=False),
...
def downBlock(in_planes, out_planes):
    block = nn.Sequential(
        SNConv2d(in_planes, out_planes, 4, 2, 1, bias=False),
#         nn.BatchNorm2d(out_planes),
        nn.LeakyReLU(0.2, inplace=True)
    )
    return block
...
def encode_image_by_16times(ndf):
    encode_img = nn.Sequential(
        ...
        SNConv2d(3, ndf, 4, 2, 1, bias=False),
        nn.LeakyReLU(0.2, inplace=True),
...
def encode_image_by_16times_reg(ndf):
    encode_img = nn.Sequential(
        # --> state size. ndf x in_size/2 x in_size/2
...
class D_NET128(nn.Module):
    def __init__(self, b_jcu=True):
        ...
        ndf = cfg.GAN.DF_DIM
        nef = cfg.TEXT.EMBEDDING_DIM
        self.img_code_s16 = encode_image_by_16times_reg(ndf)
