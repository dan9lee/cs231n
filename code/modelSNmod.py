...
def encode_image_by_16times_sig(ndf):
    encode_img = nn.Sequential(
        # --> state size. ndf x in_size/2 x in_size/2
        SNConv2d(3, ndf, 3, 1, 1, bias=True),
        nn.LeakyReLU(0.1, inplace=True), 
...
class D_NET64(nn.Module):
    def __init__(self, b_jcu=True):
        ...
        ndf = cfg.GAN.DF_DIM
        nef = cfg.TEXT.EMBEDDING_DIM
        self.img_code_s16 = encode_image_by_16times_sig(ndf)
...
class D_NET128(nn.Module):
    def __init__(self, b_jcu=True):
        ...
        self.img_code_s32_1 = Block3x3_sig(ndf * 16, ndf * 8)
        #
        if b_jcu:
            self.UNCOND_DNET = D_GET_LOGITS(ndf, nef, bcondition=False)
        else:
...
class D_NET256(nn.Module):
    def __init__(self, b_jcu=True):
        ...
        self.img_code_s64_2 = Block3x3_sig(ndf * 16, ndf * 8)
        if b_jcu:
            self.UNCOND_DNET = D_GET_LOGITS(ndf, nef, bcondition=False)
        else:
            self.UNCOND_DNET = None
